...
On first start, the backend prints a one-time bootstrap token (spec section 6.5) to its log. The entire Setup Wizard API under /v1/setup/ is locked until that token is supplied via the X-ModuLab-Bootstrap-Token header on every request; /healthz remains unauthenticated for monitoring. Implemented so far: /v1/setup/init (generates and persists the master key), /v1/setup/oidc/configure (stores the OIDC provider's issuer URL, client ID, and an encrypted client secret), and /v1/setup/group-prefix/configure (defines the OIDC groups-claim prefix, spec section 3.3). Each has a matching /status endpoint. The remaining wizard steps - DNS-challenge provider selection and the super-admin OIDC login itself - have not landed yet.
