...
In this setup the SPA is served by Vite on `http://localhost:5173`, which proxies `/v1/` and `/healthz` through to Core on `:8080` (see `frontend/vite.config.ts`) - you get hot reload, and both halves still share a single origin. Core's own port serves something else: the frontend is compiled into the Core binary with `go:embed` (`backend/internal/webui`), and in a fresh checkout that directory contains only a committed placeholder page. It has to be committed, because `go:embed` refuses to build against an empty directory - an empty one would break `go build ./...` and the `backend` CI job, which runs without a frontend build.

To have Core serve the real SPA locally - worth doing before changing anything under `internal/webui`, since it is the only way to exercise the path production actually takes:

```bash
cd frontend && npm run build && cd ..
cp -R frontend/dist/. backend/internal/webui/dist/
cd backend && go run ./cmd/core     # http://localhost:8080 now serves the real SPA
```

That overwrites the committed placeholder in your working tree. Restore it before committing, or the real `index.html` - which references asset hashes that are not in the repository - ends up committed in its place:

```bash
git checkout -- backend/internal/webui/dist/index.html
```

Everything else under `backend/internal/webui/dist/` is gitignored, and the Dockerfile runs the same copy in its `go-builder` stage, so a production image never depends on what happens to be sitting in that directory locally.
