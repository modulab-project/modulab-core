...
Core connects to Postgres directly, both in dev and in production - there is no connection pooler in front of it. After copying `.env.example`, override two values in your local `.env`: set `MODULAB_DB_PORT=5432` (the dev stack exposes Postgres directly on 5432) and `MODULAB_DB_PASSWORD=modulab-dev` (matching `docker-compose.dev.yml`'s `POSTGRES_PASSWORD`). Without this override, the backend cannot reach Postgres locally.
